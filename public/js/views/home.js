...
export const HomeView = {
  render(container, router) {
    ...
    const tickets = Math.max(0, Number(router.state.tickets || 0));
    const isTest = Boolean(router.state.campaign?.is_test);
    ...
    container.innerHTML = `
      <div class="card" style="text-align: center; padding: 28px 20px; background: #FFF;">
        ...
        <p style="font-size: 14px; color: var(--text-sub); margin-bottom: 18px;">
          ...
          정상 완료한 게임은 행사 설정에 따라 테스트 추첨에 참여할 수 있습니다.
        </p>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 20px;">
          <div style="background: var(--primary-light); padding: 12px; border-radius: 16px; text-align: center;">
            ...
            <div style="font-size: 20px; font-weight: 900; color: var(--primary);">${tickets}장</div>
          </div>
        ...
        <div>
          <button id="btn-start-jump" class="btn btn-primary" style="font-size: 15px; width: 100%; height: 52px; background: #1967D2; box-shadow: 0 4px 14px rgba(25, 103, 210, 0.25); border-radius: 14px; border: none; cursor: pointer; color: #FFF; font-weight: 700;">
            <span>${tickets > 0 ? '공룡 점프 시작' : '게임권이 없습니다'}</span>
      ...
      <div class="card" style="padding: 16px 20px;">
        <div class="card-title" style="font-size: 15px; justify-content: space-between;">
          <span>🎁 참여 리워드 안내</span>
          <span class="sticker-badge badge-blue">${isTest ? '개발용 테스트 리워드' : '행사 설정 기준'}</span>
        </div>
        <div style="display: flex; gap: 12px; overflow-x: auto; padding: 6px 0;">
          <div style="flex: 0 0 94px; text-align: center; background: #F8FAFF; padding: 10px 6px; border-radius: 14px; border: 1px solid #D2E3FC;">
            ...
            <div style="font-size: 11px; font-weight: 700; color: #1967D2;">랭킹 도전</div>
            <div style="font-size: 9.5px; color: #5F6368;">운영 정책 확정 전</div>
          </div>
          <div style="flex: 0 0 90px; text-align: center; background: #F8F9FA; padding: 10px 6px; border-radius: 14px; border: 1px solid #E8EAED;">
            <div style="font-size: 24px;">☕</div>
            <div style="font-size: 11px; font-weight: 700; color: #3C4043;">테스트 추첨</div>
            <div style="font-size: 9.5px; color: #5F6368;">가짜 재고만 사용</div>
          </div>
          <div style="flex: 0 0 90px; text-align: center; background: #F8F9FA; padding: 10px 6px; border-radius: 14px; border: 1px solid #E8EAED;">
            <div style="font-size: 24px;">🏪</div>
            <div style="font-size: 11px; font-weight: 700; color: #3C4043;">결과 복구</div>
            <div style="font-size: 9.5px; color: #5F6368;">내 수령함에서 확인</div>
          </div>
          <div style="flex: 0 0 90px; text-align: center; background: #F8F9FA; padding: 10px 6px; border-radius: 14px; border: 1px solid #E8EAED;">
            <div style="font-size: 24px;">🛵</div>
            <div style="font-size: 11px; font-weight: 700; color: #3C4043;">Gemini 혜택</div>
            <div style="font-size: 9.5px; color: #5F6368;">공식 페이지 확인</div>
          </div>
        </div>
      </div>
    `;
    ...
    // Event Listeners
    const startButton=container.querySelector('#btn-start-jump');
    ...
    ui.showModal({
      ...
      trustedHtml: `
        <div style="display: flex; flex-direction: column; gap: 10px; font-size: 14px; text-align: left;">
          ...
          <div>🎁 <strong>추첨</strong>: 서버가 정상 종료로 검증한 게임만 행사 설정에 따라 테스트 추첨에 참여할 수 있어요.</div>
        </div>
      `,
