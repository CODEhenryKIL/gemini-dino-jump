...
상태: 구현 및 기능 검증 수행, **성능 목표 미달·순간 부하 재검증 대기**
...
- 따라서 **기능 구현과 주요 운영 검증은 수행됐지만 성능 인수 기준은 실패한 상태**다. 원인 수정과 재실행이 필요하며, 추가 원격 부하 실행은 남은 실행 예산 확인이 필요할 수 있다.
...
- 최종 복원 이후 [요청 제한 회귀 검사 5/5](evidence/phase1-restored-rate-tests.txt) 통과. 88개 전체 검사는 복원 이전 기록이며 복원 뒤에는 변경된 5개 검사만 재실행했다.
...
- 원격 참가자 5,003명, 게임 세션 2,285개, 검증 완료 2,211개, 추첨 2,163개를 재확인했다.
...
**실패**다. OLD2는 오류 없이 전체 단계를 수행했지만 1초 목표를 넘었고, NEW3는 burst에서 503 오류로 중단됐다. 서버 설정 복원 후보는 마련했지만 예산 부족으로 같은 원격 부하를 다시 실행하지 못했으므로 통과로 바꿀 근거가 없다.

작업 지시서의 실패 보고 규칙에 따라 이 보고서와 원본 증거는 제출할 수 있지만, 이를 성능 목표 통과 또는 1차 최종 인수 완료로 해석하지 않는다.
...
- 실제 금액·최종 청구액·CPU 연속 측정값은 확보하지 못했다. 호출·예약 시간 상한과 측정한 DB 상태를 보고하며 추가 과금이 0원이라고 단정하지 않는다.
...
- 최종 Preview는 `fe4fe13`, 배포 ID `dpl_2fr5QRcqfRzcQU66rDk5gmT5KyQN`이며 READY다.
- [최종 불변 Preview 주소](https://dino-nanobanana-6erh3mywr-henry-kils-projects.vercel.app)의 환경·프로젝트·스키마·합성 guard·방문 통계 설정을 확인했다.
- 별도 주소는 이 최종 Preview에 연결됐다. 비로그인 HEAD 요청의 Vercel 인증 페이지 302 이동으로 접근 보호 유지를 확인했다.
- 실패한 NEW3는 별도 주소로 연결하지 않았다. 최종 Preview의 전체 부하를 새로 실행하지 않았으므로 OLD2/NEW3 결과와 구분한다.
...
| 산출물 | 위치 |
| --- | --- |
| Python 88개 검사 | [phase1-local-python.txt](evidence/phase1-local-python.txt) |
| Node 25개 검사 | [phase1-local-node.txt](evidence/phase1-local-node.txt) |
...
1. NEW3 burst 503의 DB 연결 원인을 더 좁힌다. 역할 한도·Supavisor·서버리스 인스턴스별 연결을 함께 확인하며 미확정 원인을 단정하지 않는다.
