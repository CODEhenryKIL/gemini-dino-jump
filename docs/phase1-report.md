...
추가 보완: 누락됐던 사용자 기획안을 받은 뒤 [원문 대조 검토](phase1-source-plan-review.md)에서 발견한 수령 초기 상태, Gemini 공유 목적별 집계, 관측 진행 중 구분을 `codex/phase1-followup-fixes`에서 수정했다. 후속 로컬 검사 Python 123개·Node 35개가 통과했다. **원격 DB와 Preview에는 보완 코드를 반영하지 않았다.** 아래 최초 원격 시험 기록과 [후속 보완 결과](phase1-followup-report.md)를 구분해 확인한다.

GitHub 반영: 사용자 승인으로 PR #3을 `main`에 병합했다. 병합 직후 완료된 자동 리뷰에서 점수 검증 등 추가 결함이 확인되어 `codex/phase1-review-fixes`에서 보완했다. 최신 검증과 한계는 [추가 리뷰 수정 기록](phase1-review-followup.md)에 기록한다. 아래 최초 시험 기록은 당시 배포본의 결과다.
...
- 최초 시험 종료 당시 Draft PR은 검토 상태로 유지했다. 이후 사용자 요청으로 GitHub main에 병합했으며 Production은 변경하지 않았다.
...
- 최종 DB 연결 수정 후 Python 115개가 통과했다. 최초 1차 프론트 변경 후 Node 28개가 통과했다. 이는 후속 보완 이전 기록이다. 쿠키/관리자 권한 취소 뒤 캐시 재생 차단, 환급 거절 상태 보존, 동일 참가자 동시 추첨, 쿨다운 경계, 실제 빈 DB migration 및 지표 필터 검사를 포함한다.
