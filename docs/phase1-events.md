...
| `participant_ready` | Cookie participant initialization or restore finished. |
...
| `benefit_viewed`, `gemini_cta_viewed`, `gemini_cta_clicked` | Ordered Gemini exposure and click funnel. Click is not registration. |
...
- Loading drop estimate: observations with `entry_viewed` and no later `participant_ready` or next-screen event after the observation window. Browser termination can omit the final event, so the dashboard labels this estimated.
