...
| 홈 | 기본권·초대권·최고점·복주머니 상태를 한눈에 표시한다. 진행 세션, 권리 없음, 쿨다운, 캠페인 중단·종료 상태를 구분한다. | 게임 시작 또는 진행 게임 복원 / 조작 가이드 | `GET /api/me`로 복원된 `tickets`, `best_score`, `draw`, `pending_game_session`; 캠페인 설정 | `screen_entered`, `game_cta_clicked`, `screen_left` |
...
| 초대·공유 | 일반 재도전 초대, 기록 공유, 경품 결과 공유 문맥을 분리한다. 지급·사용·환급, 유효 방문, 잔액, 쿨다운을 표시한다. | 네이티브 공유 / 링크 복사 | `GET /api/referrals/me`; `ticket_totals {granted,used,refunded}`, `invitation_balance`, `valid_visits`, `cooldown_until` | `invite_cta_viewed`, `share_attempted` |
...
- 앱 활성화와 `pageshow`에서 `/api/me`를 다시 읽어 게임권·쿨다운·추첨·진행 세션 상태를 갱신한다.
...
- 수령·TOP3 폼은 빈 값, 동의 누락, API 실패에서 현재 입력을 보존한다.
