...
- `8404e6d` Preview의 인증된 관리자 API에서 위 세 라벨을 확인했다. 기존 가이드 노출 2·클릭 3·고유 참가자 1과 긁기 보조 요청 1·고유 참가자 1이 유지됐다. 관리자 화면은 응답의 `label`을 그대로 표시하며, 이번 배포에서 새 관리자 브라우저 로그인 검증을 반복하지는 않았다.
- `record_share`의 parser·query allowlist·관찰 연결·유효 방문 보상과 실제 Preview 기록 공유 복사 집계를 확인했다. 후속 원격 조회에서도 일반 초대·기록 공유가 각각 분류됐다. 기존 합성 당첨자의 로컬 실제 수령·공유 화면을 `8404e6d` Preview API에 연결해 `prize_share` 복사 시도 1·성공 1·고유 참가자 1·미연결 0을 추가 대조했다. 신규 참가자와 재고를 만들지 않았으며, 원격 원본 도메인의 당첨자 브라우저 복원 검증과는 구분한다.
