...
- 랭킹 화면 내부의 통신 재시도는 새 `ranking_viewed`나 화면 진입으로 중복 집계하지 않는다.
...
복사·네이티브 공유의 결과가 늦게 도착해도 클릭 당시의 화면·화면 방문 ID·활성 시간을 결과 이벤트에 유지한다. 화면 이탈 뒤 결과도 원래 시도에 연결하되, 현재 화면에 이전 안내를 표시하지 않는다. 공유 창 대기 시간이나 복사 완료를 새 화면의 활동으로 계산하지 않는다. 처리 중 중복 클릭·화면 재진입·취소·실패의 로컬 비동기 회귀를 확인했다.
...
- 후속 `d2bad58`은 연결 처리 순서가 바뀌는 경합을 로컬에서 검증하고 제한된 재전송·비식별 거절 코드를 제공한다. 해당 Preview의 첫 방문과 새로고침은 참가자 1명·기본권 지급 1회로 유지됐고, 두 관찰의 초기 이벤트와 로딩 체크포인트가 모두 저장됐다. 원격에서 과거 경고의 정확한 원인을 소급 확정하거나 무누락을 보장하는 증거는 아니다.
- `a97788d`의 실제 원격 관리자 로그인 → 기본 7일 통계 → `phase2_scratch` 필터 조회를 확인했다. 긁기 진입·노출·저장 각 1명과 이탈/관찰 중 0이 API 확인값과 일치했다. 기존 합성 수령 건의 보류/확인 대기 저장은 관리자 UI와 DB 이력 2건으로 대조했으며 실제 연락·지급은 하지 않았다.

- `4b50d9c` Preview의 실제 가이드 버튼에서 두 공개 Notion 본문이 새 탭으로 열렸다. `phase2_guides` 단일 참가자의 관리자 API 집계는 콘텐츠별 노출 1·클릭 1·고유 클릭 참가자 1이었다. 공부 가이드 재클릭 뒤 클릭 2·고유 클릭 참가자 1로 유지돼 발생 횟수와 사람 수가 구분됐다. 미연결 이벤트·경유 요청은 0이며 이 수치는 단일 기능 확인 결과다.
- 화면 코드와 현재 클라이언트·서버 허용 목록에는 `content_viewed`, `content_clicked`, `scratch_reveal_requested`가 등록돼 있다. 콘텐츠 노출/클릭의 원격 저장·집계와 긁기 보조 공개·단계 집계를 위 증거로 확인했다. 관리자 카드의 새 이벤트 표시는 `가이드 카드 노출`, `가이드 링크 클릭`, `긁기 보조 공개 요청`으로 보완했으며 관련 로컬 DB 회귀 4개를 통과했다.
- `record_share`의 parser·query allowlist·관찰 연결·유효 방문 보상에 더해 관리자 기록 공유 복사 시도 1·성공 1도 확인했다. 일반 초대·경품 공유까지 모든 목적의 화면 발생→목적별 관리자 집계를 대조한 것은 아니다.
- `content_guides`의 서버 설정을 소비하는 실제 혜택 화면에서 두 HTTPS 링크의 카드 클릭 → 공개 본문 접근 → 클릭 집계를 확인했다. Notion 본문 자체의 CTA 수정·게시나 경유 추적 연결은 하지 않았다.
