...
- 화면 코드와 현재 클라이언트·서버 허용 목록에는 `content_viewed`, `content_clicked`, `scratch_reveal_requested`가 등록돼 있다. `content_viewed`의 원격 저장·콘텐츠별 노출 집계와 긁기 보조 공개·단계 집계는 위 증거로 확인했다. `content_clicked`의 최종 Preview 클릭부터 관리자 집계까지의 연결, 새 이벤트의 관리자 표시 이름·차원을 한 흐름에서 대조하는 확인은 남아 있다.
